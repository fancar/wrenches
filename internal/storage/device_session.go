...
type DeviceSession struct {
	...
	// additional kek keys for passing App Session Key between ns and join server
	AESKey   lorawan.AES128Key
	KEKLabel string

	AppSKey lorawan.AES128Key // the key we got from AS
}
...
type DeviceSessionCSV struct {
	...
	AESKey   string `csv:"AESKey"` // got from KeyEnvelope OR SQL (application-server db)
